  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, marginBottom: 20, overflow: "hidden" }}>
      ...
      <div style={{ height: 280, overflow: "hidden" }}>
        <img src={item.image_url} alt="신고 콘텐츠" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
      </div>
      <div style={{ padding: "12px 16px" }}>
        <div
          ...
          style={{
            ...
            padding: onTextSelect ? "6px 8px" : "0",
            borderRadius: 6,
            background: onTextSelect ? "rgba(124,58,237,.04)" : "transparent",
            border: onTextSelect ? `1px dashed rgba(124,58,237,.2)` : "none",
        ...
        {onTextSelect && (
          <div style={{ fontSize: 10, color: C.purple, marginTop: 6, display: "flex", alignItems: "center", gap: 4 }}>
            ✏️ 위 텍스트를 드래그하면 우회표현에 자동 입력돼요
          </div>
        )}
      ...
      <div>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden", marginBottom: 16 }}>
          ...
          <div style={{ padding: "12px 18px", display: "flex", gap: 24 }}>
            ...
            {h.elapsed != null && (
              <div><div style={{ fontSize: 10, color: C.sub, marginBottom: 3 }}>판정 소요</div><div style={{ fontSize: 12, color: C.text, fontWeight: 600 }}>{h.elapsed}초</div></div>
            )}
          </div>
        </div>
      </div>
      ...
      {records.length === 0 ? (
        ...
        return (
          <div key={i} onClick={() => setSelected(h)} style={{
            ...
            padding: "16px 18px", marginBottom: 10, display: "flex", alignItems: "center",
            gap: 14, cursor: "pointer", transition: "box-shadow .15s, border-color .15s",
          }}
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
        {steps.map((s, i) => (
          <div key={i} style={{ flex: 1 }}>
            <div style={{ height: 3, borderRadius: 99, marginBottom: 6, background: step >= i + 1 ? C.purple : C.muted, opacity: step > i + 1 ? 0.4 : 1, transition: "background .3s" }} />
      ...
      {step === 2 && (
        <div>
          ...
          <Btn onClick={() => rtype && setStep(3)} style={{ width: "100%", background: rtype ? C.purple : C.muted, color: rtype ? "#fff" : C.sub }}>다음 →</Btn>
        </div>
      )}
      ...
      {step === 3 && rtype === "TYPE1" && (
        <div>
          ...
          <label style={{ fontSize: 11, color: C.sub, display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
            우회표현 (발견된 표현)
            {dragHint && (
              <span style={{ color: C.purple, fontWeight: 700, fontSize: 11, background: "rgba(124,58,237,.08)", padding: "2px 8px", borderRadius: 6 }}>
                ✓ "{dragHint}" 입력됨
              </span>
            )}
          </label>
          <input
            ...
            placeholder="위 텍스트를 드래그하거나 직접 입력하세요"
            style={{
              width: "100%", padding: "10px 14px", borderRadius: 10,
              border: `1.5px solid ${varWord ? C.purple : C.border}`,
              background: varWord ? "rgba(124,58,237,.04)" : C.surface,
          ...
          />
          <Btn onClick={() => (varWord && bwText) && setDone(true)} style={{ width: "100%", background: (varWord && bwText) ? C.purple : C.muted, color: "#fff" }}>
            판정 완료
          </Btn>
        </div>
      )}
      ...
      {step === 3 && (rtype === "TYPE2" || rtype === "TYPE3") && (
        <div>
          ...
          {htID === 8 && (
            <textarea value={etcText} onChange={e => setEtc(e.target.value)} placeholder="기타 사유를 입력해주세요"
              style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: `1px solid ${C.border}`, background: C.surface, color: C.text, fontSize: 13, marginBottom: 12, boxSizing: "border-box", resize: "none", height: 80, outline: "none" }}
            />
          )}
          <Btn onClick={() => htID && (htID !== 8 || etcText.trim()) && setDone(true)}
            style={{ width: "100%", background: (htID && (htID !== 8 || etcText.trim())) ? TYPE_CFG[rtype].color : C.muted, color: "#fff" }}>
            판정 완료
          </Btn>
        </div>
      )}
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden", marginBottom: 14 }}>
        {[
          { v: stats.total,   u: "건", l: "전체 검토" },
          { v: stats.weekly,  u: "건", l: "이번 주"   },
          { v: stats.avgSec,  u: "초", l: "평균 검토" },
        ].map((s, i) => (
          <div key={i} style={{ padding: "20px 0", textAlign: "center", borderRight: i < 2 ? `1px solid ${C.border}` : "none" }}>
            <div style={{ fontSize: 26, fontWeight: 800, letterSpacing: -1, color: C.text }}>{s.v}<span style={{ fontSize: 12, color: C.sub, fontWeight: 400, marginLeft: 2 }}>{s.u}</span></div>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", gap: 3, background: C.muted, borderRadius: 9, padding: 3 }}>
            {tabs.map(t => (
              <button key={t} onClick={() => setTab(t)} style={{ padding: "3px 10px", borderRadius: 7, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 600, background: tab === t ? "#fff" : "transparent", color: tab === t ? C.text : C.sub, boxShadow: tab === t ? "0 1px 3px rgba(0,0,0,.08)" : "none" }}>{t}</button>
            ))}
          </div>
        </div>
        {filtered.map((h, i) => {
...
export default function BetaTesterApp() {
  ...
  const [stats, setStats] = useState({
    total:  METRIC.total_review_cnt,   // 128
    weekly: HISTORY.length,             // 7 (기존 히스토리 건수)
    avgSec: METRIC.avg_review_sec,      // 47
  });
  ...
  const handleJudgeDone = (result) => {
    if (result) {
      ...
      setStats(prev => {
        ...
        // 기존 평균에 새 elapsed를 가중 평균으로 반영
        const newAvg = result.elapsed != null
          ? Math.round((prev.avgSec * prev.total + result.elapsed) / newTotal)
          : prev.avgSec;
        ...
        <nav style={{ padding: "12px 10px", flex: 1 }}>
          {NAV.map(n => (
            <button key={n.id} onClick={() => { setPage(n.id); setJudging(null); }} style={{
              ...
              transition: "all .15s",
              borderLeft: page === n.id && !judging ? `3px solid ${C.purple}` : "3px solid transparent",
            }}>
