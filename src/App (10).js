...
function ReportCard({ item, onTextSelect }) {
  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, marginBottom: 20, overflow: "hidden" }}>
      ...
      <div style={{ height: 280, overflow: "hidden", background: C.muted, display: "flex", alignItems: "center", justifyContent: "center" }}>
    ...
    <div>
      ...
      <div style={{ position: "relative", marginBottom: 16 }}>
        ...
        <input
          ...
          placeholder="#신고번호, 내용, TYPE1, 혐오 등으로 검색"
          style={{ width: "100%", padding: "10px 14px 10px 34px", borderRadius: 10, border: "1px solid " + (query ? C.purple : C.border), background: C.surface, color: C.text, fontSize: 13, boxSizing: "border-box", outline: "none", transition: "border-color .2s" }}
        />
        {query && (
          <button onClick={() => setQuery("")} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: C.sub, fontSize: 16 }}>✕</button>
        )}
      </div>
      ...
      {searched.length === 0 ? (
        ...
        const hateName = h.htID ? HATE_TYPES.find(t => t.ht_ID === h.htID)?.ht_name : null;
        return (
          <div key={i} onClick={() => setSelected(h)} style={{ background: C.surface, border: "1px solid " + C.border, borderRadius: 12, padding: "16px 18px", marginBottom: 10, display: "flex", alignItems: "center", gap: 14, cursor: "pointer", transition: "box-shadow .15s, border-color .15s" }}
    ...
    <div>
      ...
      {step === 3 && rtype === "TYPE1" && (
        <div>
          ...
          <input value={varWord} onChange={e => setVar(e.target.value)} placeholder="위 텍스트를 드래그하거나 직접 입력하세요" style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "1.5px solid " + (varWord ? C.purple : C.border), background: varWord ? "rgba(124,58,237,.04)" : C.surface, color: C.text, fontSize: 13, marginBottom: 14, boxSizing: "border-box", outline: "none" }} />
          <label style={{ fontSize: 11, color: C.sub, display: "block", marginBottom: 6 }}>추정 원단어</label>
          <input value={bwText} onChange={e => setBw(e.target.value)} placeholder="예: 누칼협, 엠생" style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "1px solid " + C.border, background: C.surface, color: C.text, fontSize: 13, marginBottom: 20, boxSizing: "border-box", outline: "none" }} />
          <Btn onClick={() => (varWord && bwText) && setDone(true)} style={{ width: "100%", background: (varWord && bwText) ? C.purple : C.muted, color: "#fff" }}>판정 완료</Btn>
        </div>
      )}
      {step === 3 && (rtype === "TYPE2" || rtype === "TYPE3") && (
        <div>
          ...
          <Btn onClick={() => htID && (htID !== 8 || etcText.trim()) && setDone(true)} style={{ width: "100%", background: (htID && (htID !== 8 || etcText.trim())) ? TYPE_CFG[rtype].color : C.muted, color: "#fff" }}>판정 완료</Btn>
        </div>
      )}
    </div>
  );
      ...
      <div style={{ background: C.surface, border: "1px solid " + C.border, borderRadius: 14, padding: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", gap: 3, background: C.muted, borderRadius: 9, padding: 3 }}>
            {tabs.map(t => <button key={t} onClick={() => setTab(t)} style={{ padding: "3px 10px", borderRadius: 7, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 600, background: tab === t ? "#fff" : "transparent", color: tab === t ? C.text : C.sub, boxShadow: tab === t ? "0 1px 3px rgba(0,0,0,.08)" : "none" }}>{t}</button>)}
          </div>
        </div>
        {filtered.map((h, i) => {
          const tc = TYPE_CFG[h.type];
