      ...
      {queue.length === 0 ? (
        ...
        queue.map((a, i) => (
          <div key={i} style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
                ...
                <span style={{
                  ...
                  color: a.status === "진행중" ? C.yellow : C.sub, fontWeight: 600,
                }}>{a.status}</span>
            ...
            <Btn onClick={() => onStart(a)} style={{ background: C.purple, color: "#fff", flexShrink: 0, alignSelf: "center" }}>
              판정 시작
            </Btn>
          </div>
        ))
    ...
    <div>
      ...
      {step === 1 && (
        <div>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button onClick={() => {
              setDec("N");
              onDone({ report_ID: item.report_ID, content: item.content, image_url: item.image_url, type: "TYPE1", decision: "정상", agr: true, completed_at: new Date().toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" }), varWord: null, bwText: null, htID: null });
            }}
      ...
      {step === 2 && (
        <div>
          ...
          <Btn onClick={() => rtype && setStep(3)} style={{ width: "100%", background: rtype ? C.purple : C.muted, color: rtype ? "#fff" : C.sub }}>
            다음 →
          </Btn>
        </div>
      )}
      ...
      {step === 3 && rtype === "TYPE1" && (
        <div>
          ...
          <input value={varWord} onChange={e => setVar(e.target.value)} placeholder="예: ㄴㅋㅎ, 엠*생"
      ...
      {step === 3 && (rtype === "TYPE2" || rtype === "TYPE3") && (
        <div>
          ...
          {htID === 8 && (
            <textarea value={etcText} onChange={e => setEtc(e.target.value)}
          ...
          {NAV.map(n => (
            <button key={n.id} onClick={() => { setPage(n.id); setJudging(null); }} style={{
              ...
              {n.id === "detail" && newRecords.length > 0 && (
                <span style={{ marginLeft: "auto", fontSize: 10, background: C.green, color: "#fff", borderRadius: 20, padding: "1px 7px" }}>NEW</span>
              )}
            </button>
          ))}
        </nav>
